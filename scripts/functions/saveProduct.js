...
export function saveProduct(id, products) {
  if (isUserOnline()) {
    ...
    if (localStorage.getItem("cart")) {
      //verifica si existe un array de productos del cart
      cart = JSON.parse(localStorage.getItem("cart")); // actualiza el local storage con los productos existentes
      Swal.fire(
        "¡CANTIDAD ACTUALIZADA!",
        "Se actualizó la cantidad de productos en el carrito",
        "success"
      );
      const ProductIndex = cart.findIndex((item) => item.id == id);

      if (ProductIndex !== -1) {
        //verifica si el producto existe en el cart
        ...
        // si no existe en el cart, agrega un producto al array
